...
function checkDistance(x,y){
    if(boardState[`${x},${y}`] === true){
        bestDistance = Math.min(bestDistance, Math.abs(x) + Math.abs(y));
    }
}


// traceWire(p1, markPath);
// traceWire(p2, checkDistance);


// part two
function traceWireTwo(wire, cb){
    let x = 0;
    let y = 0;
    let steps = 0;
...
function markSteps(x,y,steps){
    stepState[`${x},${y}`] = steps; 
}

function checkSteps(x,y,steps){
    let prevSteps = stepState[`${x},${y}`];
    if(prevSteps != undefined){
        minSteps = Math.min(minSteps, prevSteps + steps);
    }
}


traceWireTwo(p1, markSteps);
traceWireTwo(p2, checkSteps);
